        ...
        for a_tag in all_a_tags:
            if a_tag.strip().startswith('window'):
                ...
                for node in nodes:
                    ...
                    result.append({'txt': node['caption'] if 'caption' in node.keys() else '', 'pic':  node['display_src'] if 'display_src' in node.keys() else '', 'date':  node['date'] if 'date' in node.keys() else ''})
...
if __name__ == '__main__':
    ...
    teddy_file.close()
    ret = [i for i in result if i not in done]  # 将要写入的动态
    # 对ret中的元素按date排序
    if len(ret) > 1:
        ret = make_sort(ret)
    print ret
